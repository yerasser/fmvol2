...
export default function VinylRecordCanvas() {
    const items = useMemo(
    ...
    const ensureDust = (pxSize, dpr) => {
        ...
        for (let y = 0; y < wantH; y++) {
            for (let x = 0; x < wantW; x++) {
                const n = hash2(x, y); // 0..1
                const speck = n > 0.995 ? 1 : 0;
                const grain = (n - 0.5) * 0.06;
                const v = clamp(0.0 + grain + speck * 0.55, 0, 0.8); // 0..0.8
    ...
    const draw = (ctx, angleRad) => {
        ...
        ctx.fillStyle = "rgba(0,0,0,0.0001)"; // just to trigger shadow
        ctx.fill();
        ctx.restore();
        ...
        ctx.save();
        const g = ctx.createLinearGradient(-R * 0.64, -R * 0.55, R * 0.64, -R * 0.12); // x1=18% y1=20% -> x2=82% y2=48%
        ...
        ctx.save();
        const edge = ctx.createRadialGradient(R * 0.05, -R * 0.05, R * 0.1, 0, 0, R * 0.70);
        ...
        ctx.beginPath();
        ctx.arc(0, 0, labelR - R * 0.067, 0, Math.PI * 2); // ~ labelR-10 when R~150
        ...
        for (let i = 0; i < N; i++) {
            ...
            const rText = labelR + R * 0.23; // ~ labelR+34 at 150px
            ...
            drawMultiline(ctx, items[i], 0, -R * 0.20, lineHeight);
    ...
    const spin = () => {
        ...
        const offset = margin + Math.random() * (slice - 2 * margin);
        const pointerTarget = winnerIndex * slice + offset;

        const extraTurns = 6 + Math.floor(Math.random() * 5);
        const jitter = (Math.random() - 0.5) * (slice * 0.1);

        const target = Math.PI * 2 * extraTurns - pointerTarget + jitter;

        const from = rotationRef.current;
        const to = from + target + Math.PI * 4;
