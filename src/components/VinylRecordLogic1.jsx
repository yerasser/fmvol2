...
export default function VinylRecordLogic1() {
    const items = useMemo(
    ...
    // ===== NEW: last 3 winners (no repeats) =====
    // store indices + debug info
    ...
    const tick = (now) => {
        ...
        if (t < 1) {
            ...
            console.log("[spin result]", {
                plannedIdx: lastWinnerPlannedRef.current,
                plannedName: items[lastWinnerPlannedRef.current],
                finalIdx,
                finalName: items[finalIdx],
                finalAngle: rotationRef.current,
            });
    ...
    const spin = () => {
        ...
        // ===== winner cannot repeat in last 3 spins =====
        ...
        // ===== make final angle land EXACTLY in winnerIndex sector (according to idxUnderPointer) =====
        ...
        // idxUnderPointer:
        // a = ((-angle % 2pi)+2pi)%2pi
        // shifted = (a + slice*0.03) % 2pi
        // idx = floor(shifted / slice)
        //
        // We choose shifted inside the winner slice, then invert to get desired angle modulo 2pi.
        const shifted = (winnerIndex * slice + offsetInSlice) % twoPi;
        const a = (shifted - slice * 0.03 + twoPi) % twoPi;       // undo +slice*0.03
        const desiredMod = (twoPi - a) % twoPi;                   // angle mod 2pi

        const from = rotationRef.current;
        const fromMod = ((from % twoPi) + twoPi) % twoPi;

        let deltaMod = (desiredMod - fromMod + twoPi) % twoPi;    // forward to desiredMod
        ...
        console.log(
            "[spin planned]",
            {
                ...
                blockedIdx: blockedArr,
                blockedNames: blockedArr.map((i) => items[i]),
                attempts,
                from,
                fromMod,
                desiredMod,
